...
export function validateDateValue(raw: unknown, formats: readonly DateFormat[]) {
  ...
  if (!value) return { ok: true as const }; // allow empty date fields
  ...
  for (const fmt of formats) {
    ...
    if (fmt === 'MMM-YY') {
      const m = value.match(/^([A-Za-z]{3})-(\d{2})$/);
      if (!m) continue;
    ...
    if (fmt === 'DD-MMM-YY' || fmt === 'DD-MMM-YYYY') {
      const m = value.match(/^(\d{1,2})-([A-Za-z]{3})-(\d{2}|\d{4})$/);
      if (!m) continue;
      const dd = Number(m[1]);
...
export function makeRowFingerprint(row: Record<string, string>, columns: readonly string[]): string {
  // Exact-row duplicate detection (based only on required columns, in a stable order).
  // Use a rarely-used separator to avoid accidental collisions.
  const sep = '\u0001';
  let out = '';
